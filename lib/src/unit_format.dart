...
abstract class UnitFormat {
  ...
  static final UnitFormat standardUnitFormat = new _DefaultUnitFormat();

  /// Holds the ASCII unit format.
  static final UnitFormat asciiUnitFormat = new _ASCIIUnitFormat();

  static final Map<String,UnitFormat> _localeFormats = {};

  /// Returns the unit format for the specified locale.
  static UnitFormat getInstance([String inLocale]) =>
      inLocale==null ? standardUnitFormat :
      _localeFormats.putIfAbsent(inLocale, ()=>new _DefaultUnitFormat(inLocale));
  ...
  String format(Unit unit) {
    ...
    if (unit is ProductUnit) {
      ...
      if (negExponents.length==1) return "$posExponents/${negExponents.join("·")}";
  ...
  Unit parseProductUnit(String input) => productUnitParser.end().parse(input).value;


  /// Parses a string to produce a single unit.
  Unit parseSingleUnit(String input) => singleUnitParser.end().parse(input).value;
  ...
  Unit unitFor(String name) => _nameToUnit[name] ?? SI.unitsBySymbol[name] ??
      NonSI.unitsBySymbol[name];
  ...
  static Parser<String> get _unitIdentifier => _unitIdentifierChar.plus().flatten();
...
class _DefaultUnitFormat extends UnitFormat {
  ...
  static const List<Unit> SI_UNITS = const [ SI.ampere, SI.becquerel,
  ...
  static const List<String> PREFIXES = const [ "Y", "Z", "E", "P", "T", "G",
  "M", "k", "h", "da", "d", "c", "m", "µ", "n", "p", "f", "a", "z",
  "y" ];

  static const List<UnitConverter> CONVERTERS = const [ SI.E24, SI.E21, SI.E18, SI.E15, SI.E12,
  SI.E9, SI.E6, SI.E3, SI.E2, SI.E1, SI.Em1, SI.Em2, SI.Em3, SI.Em6, SI.Em9, SI.Em12,
  SI.Em15, SI.Em18, SI.Em21, SI.Em24 ];
  ...
  _DefaultUnitFormat([String locale]) {
    for (int i = 0; i < SI_UNITS.length; i++) {
      for (int j = 0; j < PREFIXES.length; j++) {
        ...
        String symbol = si is BaseUnit ? si.symbol: (si as AlternateUnit).symbol;
    ...
    if (Intl.shortLocale(locale ?? Intl.defaultLocale ?? Intl.systemLocale)=="GB") {
...
class _ASCIIUnitFormat extends UnitFormat {
  ...
  _ASCIIUnitFormat() {
    ...
    // Alias and ASCIIFormat for Ohm
    label(SI.ohm, "Ohm");
    for (int i = 0; i < _DefaultUnitFormat.PREFIXES.length; i++) {
      label(SI.ohm.transform(_DefaultUnitFormat.CONVERTERS[i]), _asciiPrefix(_DefaultUnitFormat.PREFIXES[i]) + "Ohm");
    }
    ...
    for (int i = 0; i < _DefaultUnitFormat.PREFIXES.length; i++) {
      label(SI.celsius.transform(_DefaultUnitFormat.CONVERTERS[i]), _asciiPrefix(_DefaultUnitFormat.PREFIXES[i]) + "Celsius");
    }
